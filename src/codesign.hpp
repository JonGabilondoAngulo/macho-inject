...
bool codesign_fs_object(const std::filesystem::path & folderPath,
                        ...
                        const bool removeSignature = false);
...
bool codesign_extract_entitlements_from_app(const std::string& appPath,
                                            std::string& newEntitlementsFile,
                                            const std::string& tempDirPath);

#pragma mark - Provision functions

int codesign_copy_provision_file(const std::filesystem::path & appPath,
                                 const std::filesystem::path& argProvision);
